...
# KNN sınıflandırıcısını eğitim verileri üzerinde kullanarak modeli değerlendirme
KNN_classifier.fit(cnn_features_train, train_labels_index)

# Doğrulama seti üzerinde tahmin yapma
KNN_pred_labels_validation = KNN_classifier.predict(validation_features)

# Doğruluk değerlendirmesi (doğrulama seti için)
...
# Karmaşıklık Matrisi
conf_matrix_knn = confusion_matrix(test_labels_index, KNN_pred_labels_validation)
...
print(classification_report(test_labels_index, KNN_pred_labels_validation, target_names=class_names, zero_division=1))
